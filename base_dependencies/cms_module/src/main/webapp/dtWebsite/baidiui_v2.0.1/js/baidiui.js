...
$.fn.buijs_date = function(options) {
	$(this).map(function() {
		...
		_t.datetimepicker({
			...
			'show': function() {
				if (_positions) {
					$(".bui_date").after('<div class="bui_data_mask bui_bgc_black_f72" style="position:fixed;width:100%;height:100%;left:0;top:0;z-index:' + [$(".bui_date").css("z-index") - 1] + ';"></div>');
...
function buijs_side_show(options) {
	var defaults = {
		...
		barbtn: '',
		footer: false,
		content: '<p class="bui_ta_c bui_p_24 bui_tc_white_d48"><i class="fa fa-circle-o-notch fa-spin"></i> 正在处理中...</p>'
	};
	var setting = $.extend(defaults, options);
	var bartc;
	...
	if ($(".bui_side_mask").length == 0) {
		$('body').append('<div class="bui_side_mask bui_bgc_black_f64 bui_side_close" style="position:fixed;top:0;bottom:0;left:0;right:0;"></div>');
	};
	$(".bui_wrap").append('<div class="bui_side_panel bui_' + setting.margin + ' bui_mo bui_mo_'+setting.side+' bui_bgc_' + setting.bgc + ' bui_bsd_24" style="z-index:1" id="' + setting.setid + '">' +
		...
		'</div>' +
		'<div class="bui_mo_b bui_mo_b_h bui_mo_b_f">' + setting.content + '</div>' +
		'<div class="bui_mo_f">' + setting.footer + '</div>' +
		'</div>');
	//顶部bar
	if (setting.title == false) {
		$(".bui_side_panel .bui_mo_b").removeClass('bui_mo_b_h');
		$(".bui_side_panel .bui_mo_h").remove();
	};
	//底部按钮
	if (setting.footer == false) {
		$(".bui_side_panel .bui_mo_b").removeClass('bui_mo_b_f');
		$(".bui_side_panel .bui_mo_f").remove();
	};
	setTimeout(function() {
		$(".bui_side_panel").addClass("active");
	}, 100);
	//关闭滑动框
...
function buijs_side_close() {
	$(".bui_side_panel:last-child").removeClass('active');
	setTimeout(function() {
		$(".bui_side_panel:last-child").remove();
