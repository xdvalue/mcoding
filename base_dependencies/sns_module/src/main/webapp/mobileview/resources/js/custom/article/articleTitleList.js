...
var DTExpertList = function () {
    ...
    var initTableHeaderInfo = function(){
        var aoColumns = [
            ...
            { "sTitle": "分类名称", "mData": "title"},
            {
            	...
            	"fnCreatedCell" : function(nTd, sData, oData, iRow, iCol) {
            		if(!(oData.bannerpic)){
            			$(nTd).append("<label class='label label-info'>无封面图</label>");
            		}else{
            			$(nTd).append("<img src='" + oData.bannerpic + "' width='40%'>");            			
            		}
            	}
            },
            {
            	...
            	"fnCreatedCell" : function(nTd, sData, oData, iRow, iCol) {
            		if(oData.hide==0){
            			$(nTd).append("<label class='label label-danger'>隐&nbsp藏</label>");
            		}else{
            			$(nTd).append("<label class='label label-success'>显&nbsp示</label>");            			
            		}
            	}
            },
            { "sTitle": "跳转地址", "mData": "href"},
			{ "sTitle": "模版列表页面", "mData": "templist"},
			{ "sTitle": "内容模版页面", "mData": "tempdetail"},
			{ "sTitle": "索引模版页面", "mData": "tempindex"}
			//,
        ...
        var aoColumnDefs =[];
            aoColumns.push({
            	...
            	"sVisible" : false, 
            	"fnCreatedCell": function(nTd,sData, oData, iRow, iCol){
            		$(nTd).append("<span style='margin: 0 5px 0 5px'><span class='btn btn-xs purple ajaxify' href='page/addArticleTitle.html?id="+oData.id+"'><i class='fa fa-edit'></i> 编辑 </span>");
    ...
    var loadTableData = function(){
        ...
        oTable = $('#'+tableId).DataTable({
            ...
            "sAjaxSource": "management/queryArticleTitleByPage.html",
    ...
    var initToolBar = function(){
        ...
        $("#deleteBut").on("click", function(){
            ...
            $.ajax({
                type: "post",
                data: param,
                url: 'management/deleteArticleTitle.html',
