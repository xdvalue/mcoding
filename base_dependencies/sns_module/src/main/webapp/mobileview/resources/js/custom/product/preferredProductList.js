//产品列表
var Product_preferredProductList = function () {
	var tableId = "preferredProductTable";
	var oTable = null;
        ...
        var aoColumns = [
			{
			    ...
			    "fnCreatedCell" : function(nTd, sData, oData, iRow, iCol) {
			    	var a = $("#preferredProductId").val();//编辑时默认选择
			    	if(a!=""){
			    		preferredProductIds=a.split(",");
			    	}
			        ...
			        if(a!=""){
			        	for(var i=0;i<preferredProductIds.length;i++){
							if(oData.productId==preferredProductIds[i]){
								flag = true;
							}
						}
			        }
    ...
    var convertProductCoverImg = function(productCoverImg){
    	var html = "<img src='"+productCoverImg+"' style='width:30%'>";
    	return html;
    };
