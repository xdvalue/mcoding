var Physical_ResultLineList = function () {
	var tableId = "resultLineListTable";
	var oTable = null;
    ...
    var initTableHeaderInfo = function(){
    	var aoColumns = [
 			{ "sTitle": "ID", "mData": "id"},
 			{ "sTitle": "结果ID", "mData": "physicalresultid"},
 			{ "sTitle": "关键词ID", "mData": "investigationkeywordid"},
 			{ "sTitle": "识别行内容", "mData": "linerecognize"},
 			{ "sTitle": "识别关键词", "mData": "keywordrecognize"},
 			{ "sTitle": "关键词", "mData": "keyword"},
 			{ "sTitle": "结果描述", "mData": "resultdesc"},
 			{ "sTitle": "结果值", "mData": "resultvalue"},
 			{ "sTitle": "结果值单位", "mData": "resultunit"},
 			{ "sTitle": "正常范围最小值", "mData": "normalrangemin"},
 			{ "sTitle": "正常范围最大值", "mData": "normalrangemax"},
 			//{ "sTitle": "是否异常", "mData": "isanomaly"},
 			//{ "sTitle": "是否严重异常", "mData": "isseriousanomaly"},
 			{ "sTitle": "权重", "mData": "weight"},
 			{ "sTitle": "得分", "mData": "score"},
 			{
				"sTitle": "偏高偏低",
				"mDataProp" : "",
				"sDefaultContent" : "",
				...
				"fnCreatedCell" : function(nTd,sData, oData, iRow, iCol) {
					var ext1 = '';
					if (oData.ext1 == null || oData.ext1 == 0) {
						ext1 = '';
					} else if (oData.ext1 == 1) {
						ext1 = '正常';
					} else if (oData.ext1 == 2) {
						ext1 = '偏低';
					} else if (oData.ext1 == 3) {
						ext1 = '偏高';
					}
					$(nTd).append(ext1);
				}
			},
			{
				...
				"sVisible" : false,
				"fnCreatedCell" : function(nTd,sData, oData, iRow, iCol) {
				    if (null == oData.isseriousanomaly || !oData.isseriousanomaly) {
				    	$(nTd).append('否');
				    } else {
				    	$(nTd).append('是');
				    }
			...
			},
			{
				...
				"sVisible" : false,
				"fnCreatedCell" : function(nTd,sData, oData, iRow, iCol) {
				    if (null == oData.isanomaly || !oData.isanomaly) {
				    	$(nTd).append('否');
				    } else {
				    	$(nTd).append('是');
				    }
             ...
             aoColumns.push({
             	...
             	"fnCreatedCell": function(nTd,sData, oData, iRow, iCol){
         			$(nTd).append("<span style='margin: 0 5px 0 5px'><span class='btn btn-xs purple ajaxify'" +
         					" href='physicalEditResultLineView?id="+oData.id+"'><i class='fa fa-edit'></i> 编辑 </span>");
         			$(nTd).append("<span class='btn default btn-xs red' data-toggle='modal' " +
    ...
    var loadTableData = function(){
        ...
        oTable = $('#'+tableId).DataTable({
        	"fnServerParams": function (aoData) {
        		aoData.push({"name": "resultId", "value": $('#resultId').val()});
        	},
            "sAjaxSource": "physicalQueryResultLineList",
        ...
        $("#deleteBut").on("click", function(){
            ...
            $.ajax({
                ...
                success:function(data){
		        	...
		        	Physical_ResultLineList.loadTableData();
		        },
		        error:function(){
		        	bootbox.alert("网络异常!");
		        }
