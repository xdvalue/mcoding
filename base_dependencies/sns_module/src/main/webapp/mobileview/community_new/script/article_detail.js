		...
		success: function(data) {
            ...
            $.map(data.data.queryResult,function(item){
				...
				_htmlList+=	'<li class="bui_pt_12" onclick="articleComment(\''+item.memberName+'\','+item.id+',\''+item.memberId+'\',1)">'
							...
							+	'<ul class="bui_avg_3 conmmentImgWrap" style="margin-left: 45px;margin-bottom: 10px">'
							+	_imgList
							+	'</ul>'
...
function articleComment(parentMemberName,commentId,parentMemberId,reCall) { 
	...
	if (parentMemberName != '') {
		buijs_modal({
			title: _modalTitle,
			positions: 'bottom',
			content: '<textarea onchange="textareag()" maxlength="140" rows="6" placeholder="' + _modalTitle + '" class="bui_ipt_48 bui_block bui_bgc_white bui_ts_16" id="articleCommentTextarea"></textarea><div class=""><ul class="bui_avg_3 imgWrap"></ul></div>',
			footer: '<p style="margin-left: 14px"></p><div class="bui_m_8 bui_avg_2 bui_row_p_12">' +
				'	<li><a href="javascript:window.history.go(-1);" class="bui_btn_48 bui_block bui_bgc_white_d48 bui_tc_white_a">取消</a></li>' +
				'	<li id="submit"><a href="javascript:void(0);" class="bui_btn_48 bui_block bui_bgc_red bui_tc_white_a">发表</a></li>' +
				'</div>'
		});
	}else {
		buijs_modal({
			...
			footer: '<p style="margin-left: 14px">'+
						...
						'</p><div class="bui_m_8 bui_avg_2 bui_row_p_12">' +
				'	<li><a href="javascript:void(0);" onclick="clearData()" class="bui_btn_48 bui_block bui_bgc_white_d48 bui_tc_white_a">取消</a></li>' +
				'	<li id="submit"><a href="javascript:void(0);" class="bui_btn_48 bui_block bui_bgc_red bui_tc_white_a">发表</a></li>' +
				'</div>'
		});
	}
...
function pict4() {
	...
	$(".imgWrap").append('<li id="imgUpLoad" class=" bui_p_6"style="position: relative; height: 115px;">'
								+        '<div class="bui_bgc_black_f48" style="height: 100%"><i class="fa fa-circle-o-notch fa-spin bui_ts_24_i bui_fac_white bui_p_12" style="position: absolute;left: 50%; margin-left: -25px;top: 30px"></i></div>'
	                            +    '</li>');
    uploadCtrl= false;
    $.ajaxFileUpload({
