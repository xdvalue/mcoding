var DataTableList = function () {
    ...
    var initTableHeaderInfo = function(){
        var aoColumns = [
			...
			{ "sTitle" : "openid", "mData" : "wxMember.wxOpenid" },
			{
				"sTitle": "性别", 
				"mDataProp" : "",
				"sDefaultContent" : "",
				...
				"sVisible" : false, 
				"fnCreatedCell":function(nTd,sData, oData, iRow, iCol){
					if(!$.isArray(oData.levelList)) return $(nTd).append('未知');
					var gradeName = null;
					$.each(oData.levelList, function(index, level){
						if(level.moduleType == 100){
							gradeName = level.gradeName;
						}
					});
					
					if(gradeName){
						$(nTd).append(gradeName);
					}else{
						$(nTd).append('未知');
					}
				...
				"fnCreatedCell":function(nTd,sData, oData, iRow, iCol){
					...
					$.each(oData.levelList, function(index, level){
						if(level.moduleType == 100){
							totalPoint = level.totalPoint;
						}
					});
					
					if(totalPoint){
						$(nTd).append(totalPoint);
					}else{
						$(nTd).append('未知');
					}
        ...
        aoColumns.push({
        	...
        	"fnCreatedCell": function(nTd,sData, oData, iRow, iCol){
        		...
        		if(oData.type==3){
        			var btnUpdateByIdHtml = "<span style='margin: 0 5px 0 5px'><span class='btn btn-xs red ajaxify' href='member/service/toUpdateViewById?memberId="+oData.id+"'><i class='fa fa-edit'></i> 更改 </span>";
