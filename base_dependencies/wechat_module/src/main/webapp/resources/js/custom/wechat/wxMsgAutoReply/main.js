...
//					var content = JSON.parse(oData.keywords);
//					$(nTd).append(content.join(','));
			...
			{ "sTitle": "优先级", "mData": "priority"},
			{
        		...
        		"sVisible" : false, 
        		"fnCreatedCell":function(nTd,sData, oData, iRow, iCol){
        			$(nTd).append(oData.isDefault == '1' ? '是': '否');
        		}
        	}
			//	操作模板
			//	{
        ...
        aoColumns.push({
        	...
        	"sVisible" : false, 
        	"fnCreatedCell": function(nTd,sData, oData, iRow, iCol){
        		var btnSetDefaultHtml = "<span class='btn default btn-xs blue' >设为默认";
        		var btnSetDefaultObj = $('<span>', {
        			style : "margin: 0 5px 0 5px",
        			click: function(){ setDefalutHandler(oData.id); }
        		});
        		btnSetDefaultObj.append(btnSetDefaultHtml);
        		btnSetDefaultObj.appendTo($(nTd));
        		
        	    var btnUpdateByIdHtml = "<span style='margin: 0 5px 0 5px'><span class='btn btn-xs purple ajaxify' href='wxMsgAutoReply/service/toUpdateViewById?id="+oData.id+"'><i class='fa fa-edit'></i> 更改 </span>";
...
//            "aoSearchCols": [null, null, {"sSearch":"myfilter"}, null, null, null, null, null, null],
    ...
    var deleteHandler = function(id){
    	bootbox.confirm("确认删除吗?", function(result) {
			...
			var url = 'wxMsgAutoReply/service/deleteById?id=' +id;
    ...
    var setDefalutHandler = function(id){
    	bootbox.confirm("确定设置这个为默认吗?", function(result) {
    		if (!result) {
    			return;
    		}
    		var url = 'wxMsgAutoReply/service/setDefaultById?id=' +id + '&originId=' + $('#originId').val();
