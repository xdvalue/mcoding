...
* Benson
* */
function kindEditor_init(param){
	$('.kindEditor').each(function () {
		var iptName = $(this).attr('name');
		kindEditorCreate(iptName,param);
	})
}
...
* Benson
* */
function kindEditor_imageUpload_init(){
	$('.kindeditorMageUpload').empty();
	$('.kindeditorMageUpload').each(function () {
		var upImgInputId = $(this).prev().attr('id');
		var upImgBtnId = $(this).attr('id');
		imageUpload({
			imageUploadBtn : upImgBtnId,
			showUrlId :  upImgInputId
		});//图片上传的按钮操作方法
	})
}
...
function touchspin_init(){
	$('.touchspin').each(function () {
		var touchspinBtnId = $(this).attr('id');
		$('#'+touchspinBtnId).TouchSpin({
